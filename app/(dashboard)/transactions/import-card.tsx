...
export const ImportCard = ({
  ...
  const handleContinue = () => {
    ...
    const arrayOfData = mappedData.body.map(
      (row) => {
        return row.reduce(
          (acc: any, cell, index) => {
            const header =
              mappedData.headers[index];
        ...
        <CardHeader className="gap-y-2 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col lg:flex-row gap-y-2 items-center gap-2">
            <Button
              size="sm"
              onClick={onCancel}
              className="w-full lg:w-auto bg-rose-500 hover:bg-rose-600"
            >
              Cancel
            </Button>
            <Button
